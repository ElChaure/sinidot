...
class UiController extends Controller
{
    ...
    public function actionLogin()
    {
        ...
        if (isset($_POST[CrugeUtil::config()->postNameMappings['CrugeLogon']])) {
            $model->attributes = $_POST[CrugeUtil::config()->postNameMappings['CrugeLogon']];
               ...
               $usuario_entra=$model['username'];
               
               $command=$connection->createCommand("SELECT count(*) from cruge_user where username='".$usuario_entra."' and currentsessioncounter=1");
               $existe = intval($command->queryScalar());
               if($existe > 0){
                    Yii::app()->user->setFlash('error', "Usuario ya esta logueado!");
            ...
            if ($model->validate()) {
   		    ...
   		    $connection=Yii::app()->db;		
                    ...
                    Yii::app()->db->createCommand('UPDATE cruge_user set currentsessioncounter=1 WHERE iduser ='.$id)->query();
                    
                    $est_user=Yii::app()->db->createCommand('SELECT estatus FROM cruge_user WHERE iduser ='.$id)->query();
		    foreach($est_user as $row): 
			$estatus= $row['estatus'];
		    endforeach;
  		    ...
  		    Yii::log(__CLASS__ . "\nCrugeLogon->login() returns true\n", "info");
                    ...
                    if($estatus==2) {  
                       $this->redirect(Yii::app()->user->returnUrl);
                          ...
                          $this->redirect(Yii::app()->request->baseUrl."/index.php?r=site/perfil");
                   }  
                } else {
    ...
    public function actionLogout()
    {
       ...
       $id = Yii::app()->user->id;  // el estandar de Yii
       $connection=Yii::app()->db;
       Yii::app()->db->createCommand('UPDATE cruge_user set currentsessioncounter=0 WHERE iduser ='.$id)->query();	
